...
/* 7: Review gained publishConsent and OfferLine gained displayLabel. A store
   ...
   *missing*, so `expenses` would arrive from the defaults — the new screens
   ...
   added it to. */
/* 31: three shape changes landed on three branches, each of them numbering
   itself 30 — so this is 31 and all three reasons still apply, because a blob
   from 29 is missing every one of them.
   ...
   And the third: `Booking` gained `work` — what the person on the job
   *reported* at check-out, a `WorkEntry` per person — where the labour rows
   above record what those hours *cost*. `assigneeId` also became something a
   screen can write.
...
interface StoreState {
   ...
   * deactivated — that nothing on screen could reach. The same gap closes the
   * field app's: assign a job to Yusuf, switch to «Mitarbeitende:r», and
   * without this you land in Marta's day with the job you just made nowhere
   * on it.
   */
