...
export const ADMIN_PERMISSIONS = [
  ...
  'catalogue',
  /*
   * The Ratgeber, and only the Ratgeber.
   *
   * This was `website` for one wave, gating a screen that edited every string
   * on the site. That screen is gone — the marketing copy lives in
   * `src/messages` and `src/content` and is a developer's to change, because
   * the pages are built at deploy time and an editor over them could never put
   * a word in front of a visitor any sooner.
   *
   * What survived is the part that is a *record* rather than a string: an
   * article has an author, a status, a publication date and a life of its own.
   * So the right is named after the thing it actually opens.
   */
  'blog',
  'addons',
  'coupons',
