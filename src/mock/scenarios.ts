    ...
    SERVICE_SLUGS.forEach((slug, si) => {
      ...
      if (status === 'accepted') {
        ...
        if (bookingStatus === 'invoiced' || bookingStatus === 'closed') {
           ...
           * screen that prints how it was settled. QR-bill flat rather than
           * rotated: §10 puts a slip on every invoice, so it is the honest
           * default for a generated row — the other two routes are carried by
           * the hand-written records, where the story behind each is visible.
           */
