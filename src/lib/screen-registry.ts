...
export const TRACKS: Track[] = [
  ...
  },
  {
    ...
    screens: [
        ...
        'The invoice list’s mirror, deliberately — same toolbar, same filters, same row menu. No draft state: a supplier’s bill arrives finished. Deleting is allowed at any status, which is where it parts company with an invoice — nobody outside the company has ever held one',
      ),
      done(
        '71d',
        'Ausgabe erfassen',
        ...
        ['new', 'unsaved changes', 'field error', 'already paid'],
        'Stages a draft and writes on the button. Settling is not a field on it — the payment route is asked for in its own dialog on the list, so a receipt cannot flip to «bezahlt» while somebody is fixing a typo in the supplier name',
      ),
  ...
  },
  {
    ...
    screens: [
        ...
        'Was H7. Carries the count of what a deactivation would leave alone — which is also what makes delete refuse',
      ),
      done(
        'U3',
        'Benutzer anlegen',
