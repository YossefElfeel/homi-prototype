...
export const TRACKS: Track[] = [
  {
    ...
    screens: [
      ...
      done('2', 'Leistung (Vorlage ×7)', 'Service template (×7)', '/services'),
      done('3', 'Preise', 'Pricing', '/pricing'),
      done('4', 'Abos', 'Plans', '/plans', ['table ≥lg', 'cards <lg']),
      done('5', 'Referenzen', 'Gallery', '/work', ['empty (default)', 'populated']),
      done('6', 'Einzelne Arbeit', 'Single work', '/work', [], 'Expanded dialog inside the gallery grid'),
      done('7', 'Über uns', 'About', '/about', ['insurance on', 'insurance off']),
      done('8', 'Kontakt', 'Contact', '/contact', ['sending', 'success', 'field error']),
      done('9', 'Gebiet (Vorlage ×8)', 'Region template (×8)', '/areas/kuesnacht'),
      done(
        'R1',
        'Blog — Übersicht',
        'Blog — index',
        '/blog',
        ['articles present (default)', 'nothing written (fresh)'],
        'The section exists to be found by search: «Wohnungsabgabe Checkliste» is what people type, not «Homivaro»',
      ),
      done(
        'R2',
        'Blog — Beitrag',
        'Blog — article',
        '/blog/wohnungsabgabe-checkliste',
        ['with a related service', 'without one'],
        'Sections rather than markdown, so every heading and paragraph is separately editable and translatable',
      ),
      done('10', 'Rechtliches (Vorlage)', 'Legal template', '/legal/agb', [
        'AGB',
        'Datenschutz',
        'Impressum',
      ]),
      done('11', '404', '404', '/diese-seite-gibt-es-nicht'),
      done('12', 'Danke', 'Thank you', '/thank-you'),
      done(
        '13',
        'Bau & Ausbau',
  ...
  },
  {
    ...
    screens: [
      ...
      done('C3', 'Bewerbung — Schritt 1', 'Application — step 1', '/jobs/apply', [
        'field error',
        'no permit',
      ], 'The work permit is the first question, not the last'),
      done('C4', 'Bewerbung — Schritt 2', 'Application — step 2', '/jobs/apply', [
        'upload',
        'file too large',
      ]),
      done('C5', 'Bewerbung eingegangen', 'Application received', '/jobs/apply/sent'),
  ...
  },
  {
    ...
    screens: [
      done('13', 'Leistung wählen', 'Choose service', '/request/service', [
        'windows count',
        'furniture count',
      ]),
      done('14', 'Gespeichertes Objekt', 'Saved property', '/request/property', ['empty', 'unmeasured'], 'Visible when the demo role is "Kunde" — switch role in the demo bar. Under Büroreinigung only the offices are listed; a stored address with no floor area is refused for a service priced on one'),
      done(
        '15',
        'Neues Objekt',
        'New property',
      ...
      done('18', 'Zutritt', 'Access method', '/request/access', [
        'present',
        'key left',
        'key box',
        'other person',
      ...
      done('19', 'Wunschtermin', 'Preferred time', '/request/slot', [
        'closed day',
        'too soon',
        'flexible',
      ]),
      done('20', 'Fotos & Notizen', 'Photos & notes', '/request/photos', [
        'uploading',
        'bad format',
        'too large',
      ]),
      done('21', 'Kontaktdaten', 'Contact details', '/request/contact', ['field error']),
      done('22', 'Prüfen & senden', 'Review & submit', '/request/review', [
        'sending',
        'confirmation',
        'out of area',
      ]),
  ...
  },
  {
    ...
    screens: [
      done('23', 'Offerte', 'Quote', '/quote/off_1'),
      done(
        '24',
  ...
  },
  {
    ...
    screens: [
      ...
      done('39', 'Meine Rechnungen', 'My invoices', '/account/invoices', ['empty', 'searched', 'filtered', 'nothing found', 'sorted'], 'Five invoices — one page, so paging is the one list state this screen cannot show'),
      ...
      done('48', 'Nachrichten', 'Messages', '/account/messages', ['empty', 'filtered by kind', 'searched', 'nothing found', 'single kind (no tabs)'], 'Four threads on the demo account, one of each kind — a request, a quote, a job and an invoice — so every tab in the strip has something under it. The rail pages at ten, which four threads never reach: the line under it states the capacity and the pager stays out of sight'),
      done('49', 'Profil & Benachrichtigungen', 'Profile & notifications', '/account/profile'),
    ],
  },
  {
    id: 'admin',
    ...
    screens: [
      ...
      done('55', 'Vorschau & senden', 'Preview & send', '/admin/requests/req_2/quote/send', [
        'sending',
        'sent',
      ]),
      done(
      ...
      done('59', 'Kalender — Woche', 'Calendar — week', '/admin/calendar', ['closure period'], 'View switcher on the calendar — a calendar is one screen with four views, not four screens'),
      done(
        '60',
        'Kalender — Monat',
        'Calendar — month',
      ...
      done('61', 'Agenda', 'Agenda list', '/admin/calendar', ['empty', 'filtered to nothing']),
      done(
        '63',
        'Buchungs-Detail',
        'Booking detail',
        '/admin/bookings/bkg_1',
        [
          'codes masked',
      ...
      done('64', 'Kunden', 'Customers', '/admin/customers', [
        'empty',
        'no search result',
      ...
      done('67', 'Objekt-Detail', 'Property detail', '/admin/properties/prp_1', [
        'codes masked',
        'codes revealed',
      ]),
      done(
      ...
      done('69', 'Abos', 'Plans', '/admin/subscriptions', ['empty', 'search', 'retired filtered']),
      done('69a', 'Abo anlegen', 'Add plan', '/admin/subscriptions/new', ['blank']),
      done('69b', 'Abo bearbeiten', 'Edit plan', '/admin/subscriptions/pln_basic/edit', [
        'existing plan',
      ]),
      done('70', 'Abo-Detail', 'Plan detail', '/admin/subscriptions/pln_basic', [
        'on sale',
      ...
      done('70a', 'Abonnent', 'Subscriber', '/admin/subscriptions/pln_basic/sub_2', [
        'active',
        'paused',
        'expired',
        'renewed once',
      ], 'One customer on one plan — history, payments and the cancellation rule'),
      done('71', 'Rechnungen', 'Invoices', '/admin/invoices', [
        'empty',
      ...
      done('72', 'Rechnung bearbeiten', 'Edit invoice', '/admin/invoices/inv_draft', [
      ...
      done('79', 'Textvorlagen', 'Message templates', '/admin/templates', [
        'translation gap',
        'vollständig',
      ...
      done('79a', 'Vorlage bearbeiten', 'Edit template', '/admin/templates/new', [
        'new',
        'translation gap',
        'vollständig',
        'SMS zu lang',
      ...
      done('81', 'Einstellungen — Zeiten', 'Settings — hours & closures', '/admin/settings', [
        'closure period',
      ]),
      done('82', 'Einstellungen — Gebühren', 'Settings — fees & rules', '/admin/settings', [
        'insurance on/off',
      ]),
        ...
        'What /contact sends. Until this wave the form validated six fields and threw the message away',
      ),
      done(
        'R3',
        'Blog verwalten',
        ...
        'Beitrag schreiben',
        'Write an article',
        '/admin/blog/wohnungsabgabe-checkliste',
  ...
  },
  {
    ...
    screens: [
      done('H1', 'Bewerbungen', 'Applications', '/admin/applications', [
        'empty',
        'owner only',
        'retention expiring',
      ...
      done('H3', 'Stellen', 'Jobs', '/admin/postings', ['six seeded', 'empty']),
      done('H4', 'Stelle bearbeiten', 'Edit job', '/admin/postings/reinigungskraft-teilzeit', [
        'published',
        'draft',
      ]),
      done(
  ...
  },
  {
    ...
    screens: [
      ...
      done('U4', 'Benutzer bearbeiten', 'Edit user', '/admin/users/tm_sandra/edit', [
        'owner role locked',
        'role change warning',
      ]),
      done(
  ...
  },
  {
    ...
    screens: [
      done('85', 'Heutige Einsätze', 'Today’s jobs', '/job', ['no jobs'], 'Scenario "hiring" + role "Team member" — today only, plus a glance at tomorrow'),
      done(
        '86',
        'Einsatz-Detail',
        'Job detail',
      ...
      done('88', 'Kein Zutritt melden', 'No access', '/job/bkg_1/no-access', ['no access logged'], 'The wait comes before the form — the fee only holds up if it happened'),
    ],
  },
];
