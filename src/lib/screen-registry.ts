...
export const TRACKS: Track[] = [
  ...
  },
  {
    ...
    screens: [
      ...
      done('45', 'Zahlungsmittel', 'Payment methods', '/konto/zahlungsmittel', ['TWINT unavailable'], 'The four «add a method» tiles are real cards now rather than buttons wearing a copy of the card treatment'),
      done('46', 'Bewertung schreiben', 'Write a review', '/konto/bewertung', ['nothing to review', 'sent']),
      done('47', 'Vorher / Nachher', 'Before & after', '/konto/fotos', ['empty', 'consent off'], 'Pages at ten visits; the busiest scenario seeds two, so the line states the capacity and the pager itself stays out of reach'),
      done('48', 'Nachrichten', 'Messages', '/konto/nachrichten', ['empty', 'searched', 'nothing found'], 'Three threads on the demo account. Search only — the threads stay bubbles, so there is no table state to show. Pages at ten, which three threads never reach'),
