...
export default function AccountRequestPage({
      ...
      <EmptyState
        ...
        action={
          <Button asChild variant="secondary">
            <Link href="/account/requests">{t('back')}</Link>
          </Button>
        }
      />
    );
   ...
   * said «Offerte erhalten» here and on the list, while /account/quotes and the
  ...
  return (
    <div>
      <PageHeader
        back={{ href: '/account/requests', label: t('back') }}
      ...
      {cancelled ? (
        <Card className="mb-app-section">
          <CardHeader
            ...
            actions={
              <Button asChild variant="secondary">
                <Link href="/request">{t('cancelledAction')}</Link>
              </Button>
            }
          />
        </Card>
                ...
                */}
                {countUnits(service).map((unit) =>
                  request.unitCounts?.[unit.id] ? (
                    <DetailRow
                      key={unit.id}
                      label={unitWords(unit, locale).noun ?? st('windowsSummary')}
                    >
