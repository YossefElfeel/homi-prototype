...
export default function SubscriberPage({
  ...
  if (!subscription || !plan) {
    return (
      <div>
        <PageHeader title={t('missingTitle')} back={{ href: '/admin/subscriptions', label: t('backAll') }} />
  ...
  return (
    <div>
      <PageHeader
        ...
        back={{ href: `/admin/subscriptions/${plan.id}`, label: t('backToPlan') }}
        meta={
          <>
        ...
        lead={
          <>
            <Link href={`/admin/subscriptions/${plan.id}`} className="hover:underline">
              {plan.name[locale]}
            </Link>{' '}
      ...
      {state === 'expired' && (
        <div className="mb-app flex gap-3 border-l-2 border-rule bg-sunken rounded-[var(--radius-lg)] p-5">
