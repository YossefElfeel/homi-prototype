...
export default function PlanDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!plan) {
    return (
      <div>
        <PageHeader title={t('missingTitle')} back={{ href: '/admin/subscriptions', label: t('back') }} />
  ...
  const columns: Column<Subscription>[] = [
    {
      ...
      cell: (s) => (
        <Link
          href={`/admin/subscriptions/${plan.id}/${s.id}`}
  ...
  return (
    <div>
      <PageHeader
        ...
        back={{ href: '/admin/subscriptions', label: t('back') }}
        meta={
          <>
          ...
          <Card pad="none" className="overflow-hidden">
            ...
            <dl className="grid gap-px border-t border-line-subtle bg-line-subtle sm:grid-cols-2">
              ...
              <Fact label={t('service')}>
                <Link href={`/admin/services/${plan.serviceSlug}`} className="hover:underline">
                  {serviceName}
                </Link>
      ...
      <section className="mt-app-section">
        ...
        */}
        ...
        <DataView
          ...
          rowActions={(s) => (
            <RowActions>
              <RowAction href={`/admin/subscriptions/${plan.id}/${s.id}`} label={t('rowView')}>
            ...
            query.trim() ? (
              ...
              <EmptyState
                ...
                action={
                  plan.active ? (
                    <Button asChild variant="secondary">
                      <Link href="/admin/requests/new">{t('subscribersEmptyAction')}</Link>
                    </Button>
                  ) : undefined
                }
              />
