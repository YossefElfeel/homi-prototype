 ...
 *
 * Screen 63 — the booking detail — moved here from `/admin/calendar/[id]`
  ...
  return (
    <div>
      ...
      <DataView
        ...
        onSelect={(b) => router.push(`/admin/bookings/${b.id}`)}
        caption={t('title')}
        openLabel={t('rowOpen')}
        empty={
          bookings.length === 0 ? (
            <EmptyState
              ...
              action={
                <Button asChild variant="secondary">
                  <Link href="/admin/quotes">{t('emptyAction')}</Link>
                </Button>
              }
            />
          ) : (
        ...
        rowActions={(b) => {
          ...
          return (
            <RowActions>
              <RowAction href={`/admin/bookings/${b.id}`} label={t('rowOpen')}>
              ...
              */}
              {!SETTLED.includes(b.status) && (
                <RowAction
                  href={`/admin/bookings/${b.id}?action=reschedule`}
              ...
              {!SETTLED.includes(b.status) && assignableTeam(team).length > 1 && (
                <RowAction
                  href={`/admin/bookings/${b.id}?action=assign`}
                  label={t('rowAssign')}
                >
