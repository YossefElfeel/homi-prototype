...
export default function CustomerHistoryPage({
  ...
  if (!customer) {
    return (
      <EmptyState
        ...
        body={t('emptyBody')}
        action={
          <Button onClick={() => router.push('/admin/customers')}>{ct('back')}</Button>
        }
      />
    );
  }
  ...
  return (
    <div>
      ...
      <PageHeader
        back={{ href: `/admin/customers/${customer.id}`, label: t('back') }}
        title={t('title')}
        meta={
          <RecordLink href={`/admin/customers/${customer.id}`} className="text-xl">
            {name}
          </RecordLink>
        }
        lead={t('lead')}
