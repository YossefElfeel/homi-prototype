...
export default function InvoiceDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    return (
      <div className="mx-auto max-w-3xl">
        <EmptyState
          ...
          action={
            <Button asChild>
              <Link href="/admin/invoices">{t('back')}</Link>
            </Button>
          }
        />
      </div>
  ...
  function reissue() {
    ...
    router.push(`/admin/invoices/${replacementId}`);
  }
  ...
  return (
    <div>
      <PageHeader
        back={{ href: '/admin/invoices', label: t('back') }}
      ...
      <p className="-mt-4 mb-app text-ink-secondary">
        ...
        {invoice.subscriptionId && (
          <>
            {' · '}
            <Link
              href={`/admin/subscriptions/${invoice.subscriptionId}`}
              className="underline decoration-from-font underline-offset-4"
            >
              {t('subscriptionLink')}
            </Link>
