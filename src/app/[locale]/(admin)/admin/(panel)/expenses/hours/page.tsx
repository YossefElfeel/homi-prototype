 ...
 *
 * A separate screen rather than a second mode of /admin/expenses, because the
...
export default function WorkforcePage() {
   ...
   * makes «Arbeitszeit» here and the «Arbeitszeit» row on /admin/finance the
  ...
  const createButton = (
    <Button asChild>
      <Link href="/admin/expenses/new?kategorie=arbeitszeit">
  ...
  function entryMenu(entry: LabourExpense) {
    return (
      <RowActions>
        <RowAction href={`/admin/expenses/${entry.id}`} label={t('rowOpen')}>
          <ActionIcon.open aria-hidden />
        </RowAction>
        {/* Rule 2 in `action-icons.ts`: a cross-link wears the record type's
            own glyph, never the eye. */}
        <RowAction href={`/admin/bookings/${entry.bookingId}`} label={t('rowJob')}>
  ...
  return (
    <div>
      <PageHeader
        title={t('title')}
        lead={t('lead')}
        back={{ href: '/admin/expenses', label: t('back') }}
        actions={
          <>
          ...
          <Card className="mb-app-section">
            ...
            <CardBody>
              <DataView
                ...
                stickyHeader={false}
                getKey={(e) => e.id}
                onSelect={(e) => router.push(`/admin/expenses/${e.id}`)}
            ...
            <Card className="lg:col-span-6">
              ...
              <CardBody>
                <DataView
                  ...
                  stickyHeader={false}
                  getKey={(r) => r.workerId}
                  onSelect={(r) => router.push(`/admin/users/${r.workerId}`)}
            ...
            <Card className="lg:col-span-6">
              ...
              <CardBody>
                <DataView
                  ...
                  onSelect={(r) => router.push(`/admin/bookings/${r.bookingId}`)}
                  caption={t('jobsTitle')}
                  openLabel={t('rowJob')}
                  empty={
                    <EmptyState
