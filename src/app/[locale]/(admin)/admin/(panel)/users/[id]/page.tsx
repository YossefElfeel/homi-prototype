...
export default function AdminUserPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function copyLink() {
    ...
    /* Absolute, because the link leaves this browser — it gets pasted into a
       chat or read out over the phone, and «/password?token=…» is not
        ...
        actions={
          <div className="flex flex-wrap gap-2">
            <Button asChild variant="secondary">
              <Link href={`/admin/users/${member.id}/edit`}>{t('editAction')}</Link>
            </Button>
            {!isOwner && (
              <Button asChild>
                <Link href={`/admin/users/${member.id}/permissions`}>
          ...
          actions={
            isOwner ? undefined : (
              <Button asChild size="sm" variant="ghost">
                <Link href={`/admin/users/${member.id}/permissions`}>{t('rightsAction')}</Link>
              </Button>
            )
          }
        />
          ...
          <div className="mt-6 border-t border-line-subtle pt-5">
            ...
            {history.logEntries > 0 && (
              <Button asChild variant="link" className="mt-2">
                <Link href={`/admin/changelog?q=${encodeURIComponent(fullName(member))}`}>
        ...
        <Card className="mt-app-section">
          ...
          <CardBody>
            ...
            <Button asChild variant="secondary" className="mt-5">
              <Link href="/admin/expenses/hours">{t('labourLink')}</Link>
            </Button>
          </CardBody>
        </Card>
      )}
