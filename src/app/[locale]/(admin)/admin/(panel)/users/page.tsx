...
export default function AdminUsersPage() {
  ...
  function reset(m: TeamMember) {
    ...
    router.push(`/admin/users/${m.id}#passwort`);
  }
  ...
  const list = (
    <DataView
      ...
      getKey={(m) => m.id}
      caption={t('title')}
      onSelect={(m) => router.push(`/admin/users/${m.id}`)}
      rowActions={(m) => {
        ...
        return (
          <RowActions>
            <RowAction href={`/admin/users/${m.id}`} label={t('rowOpen')}>
              <ActionIcon.open aria-hidden />
            </RowAction>
            <RowAction href={`/admin/users/${m.id}/edit`} label={t('rowEdit')}>
            ...
            <RowAction href={`/admin/users/${m.id}/permissions`} label={t('rowRights')}>
