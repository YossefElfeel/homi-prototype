...
const ENTRIES: Entry[] = [
  ...
  },
  {
    ...
    decision:
      'ANSWERED, by building the wrong version first and removing it. A content screen covering every string in `src/messages` and `src/content` — 36 places, some 4,700 blocks, filed by page and editable per language — shipped and was deleted a wave later. The reason it failed is not that it was badly built; it is that the marketing pages are rendered at deploy time, so nothing typed into it could reach a visitor sooner than a developer editing the file, and it asked the office to maintain a second copy of every sentence in the product for that. What stayed is the half that is a **record** rather than a string: the blog, where an article has an author, a status, a publication date and a life of its own, and the service area, where a municipality drives the coverage gate and the scheduler. The rule this leaves behind is worth stating, because the next request will sound the same: the panel manages records; prose that the site renders stays in files.',
    screens: 'R3, R4, 80',
    state: 'decided',
  },
  {
    ref: '§18a',
    question: 'Where do the homepage testimonials come from?',
    decision:
      'OPEN, and three of the five cards were removed rather than left standing. They arrived with the design build verbatim and they are testimonials for a web agency — «impressed with the UI work delivered by Developios», «Faraz was super responsive», an LMS, a Notion backend, customers in Saudi Arabia, the United States and Ghana — rendered live on the homepage of a Swiss cleaning company under a heading that says «Kundenstimmen», with five stars and a photograph beside each. They were not rewritten into three plausible Swiss households: fabricated praise on a live marketing page is a worse problem than the one it fixes. Two were written for Homivaro and stay; the third is the prototype’s one genuinely published review, `rev_published`, which carries recorded consent. The real answer is that this block should read the store’s `reviews` — the entity that already models `publishConsent`, moderation and the owner’s reply — instead of being a hand-kept array sitting beside it. That also settles the harder half: what the homepage shows on the day a company has no reviews yet. The «160+» and «Basierend auf 200+ Bewertungen» above the cards are the same question and are also not counted from anything.',
    screens: '1',
    state: 'open',
  },
  {
  ...
  },
  {
    ...
    decision:
      'OPEN, and the whole cost side rests on the answer. Every expense in this build is entered by hand — that is fine for the garage bill that arrives once a quarter and it is not fine for the rent, the insurance and the three subscriptions, which are the same six figures every month for ever. They carry a `recurring` flag today and the analytics read it, so «was läuft weiter, auch wenn der Monat leer ist» is answerable; what nothing does is write next month’s copy. A `RecurringExpense` with no engine behind it would be a record promising an automation the app does not have, so the decision comes first and it is a product decision, not a schema one: does the office want next month raised automatically the day it falls due, or a reminder on the dashboard that it is due and nobody has entered it. The second is less code and more honest — an expense that appears without anybody looking at it is one nobody checks against the bank. Wages were the other half of this question and are now half-answered: «Arbeitszeit» is a category with a record behind it — one person, one job, the hours, who paid and who carries it — so the profit figure has people in it that can be attributed rather than a lump nobody can query. «Löhne» stays for the payout that genuinely has nothing behind it. What has not moved is the typing: somebody still opens the form once per pair of hands per job, which is fine for three contractors and is not a payroll.',
    screens: '71b, 71c, 71d, 71e',
    state: 'open',
  },
  {
    ref: '§10.3c',
    question: 'Where do the hours on a labour cost come from — and what is an hour worth?',
    decision:
      'OPEN in two halves, and the second is the one that will bite. Where the hours come from: the field interface already stamps a check-in and a check-out, so a finished job is holding the real span, and the form offers it as the opening figure rounded to the quarter. Offered, not written — a cleaner who forgets to check out would otherwise book an eleven-hour day, and the person entering the cost is the one who knows. That leaves the office typing one entry per person per job. The cheap next step is a reminder that a finished job carries no hours; the expensive one is a timesheet, and it needs the business to say whether a job should raise its own labour entry the moment it is closed. An entry that appears without anybody looking at it is one nobody checks against the bank — the same argument as the recurring costs above. The second half: there is no rate anywhere. The «Ø Stundensatz» on 71e is the amount typed divided by the hours typed, so two entries for the same person in the same week can carry different rates and nothing says which is right, and «what do we pay Marta» cannot be answered from the app at all. Putting a rate on `TeamMember` fixes the arithmetic and forces the real question first: does the rate belong to the person, to the service, or to the contract — a move-out clean and an office contract are not paid the same, and a Saturday is not paid like a Tuesday.',
    screens: '71c, 71d, 71e, H7',
    state: 'open',
  },
  {
    ref: '§10.3d',
    question: 'Should the owner’s own hours be bookable as labour?',
    decision:
      'OPEN, and the form currently says yes while the seed says no. /admin/finance states plainly that the owner’s own pay is not in the costs — a sole proprietor draws from the profit, so a salary beside the rent would count the same money twice — and that sentence is the basis a reader uses to interpret the margin. «Arbeitszeit» can now break it: the person select offers the owner like anybody else, so one entry booked to Marco puts his own pay into the cost side and the profit line quietly starts meaning something different. The screen has been amended to say «ausser er wird als Arbeitszeit auf einen Einsatz gebucht», which is honest and is not a decision. The seeded data deliberately books hours only to the two contractors, so the sentence holds as shipped. Three ways out and the business owns the choice: leave it open and accept that the margin depends on a habit; take the owner out of the select, which is wrong the day he is genuinely paid for a job by a partner firm; or keep him in and show a second line on 71b for what the owner drew — which is the same second line §10.3b already asks about, and answering both at once is probably the real move.',
    screens: '71b, 71d, 71e',
    state: 'open',
  },
  {
